       ...
       //     PanelTemp.Hide();
        ...
        private async void Search_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if ((Search.Text != "")&&(e.KeyCode == Keys.Enter))
            {
                if (TypeSearch == 0)
                {
                    sql = string.Format("SELECT PHONG.SOPHONG, LOAIPHONG, TRANGTHAI, NGAYDEN, NGAYDI, HOTENKH, THOIGIANNHAN FROM PHONGTHUE JOIN KHACHHANG ON KHACHHANG.MAKH = PHONGTHUE.MAKH RIGHT JOIN PHONG ON PHONG.SOPHONG = PHONGTHUE.SOPHONG WHERE  SOPHONG LIKE N'%{0}%'", Search.Text);
                }
                else
                    sql = string.Format("SELECT PHONG.SOPHONG, LOAIPHONG, TRANGTHAI, NGAYDEN, NGAYDI, HOTENKH, THOIGIANNHAN FROM PHONGTHUE JOIN KHACHHANG ON KHACHHANG.MAKH = PHONGTHUE.MAKH RIGHT JOIN PHONG ON PHONG.SOPHONG = PHONGTHUE.SOPHONG WHERE TRANGTHAI = '" + TypeSearch + "' AND SOPHONG like N'%{0}%'", Search.Text);
           //     PanelTemp.Show();
                int x = await (LoadPhong(sql));
            //    PanelTemp.Hide();
                Animator.ShowSync(PanelNoiDung);
