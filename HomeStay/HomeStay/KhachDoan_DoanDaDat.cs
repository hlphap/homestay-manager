...
namespace HomeStay
{
    public partial class KhachDoan_DoanDaDat : UserControl
    {
        ...
        private void ButtonTimKiem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    sql = "SELECT khachhang.MAKH AS'Mã khách hàng',HOTENKH AS'Họ Tên', MADK AS 'Mã đăng ký', LOAIPHONG AS'Loại phòng', NGAYDEN AS'Ngày đến', NGAYDI AS 'Ngày đi' FROM PHONGTHUE, PHONG, KHACHHANG WHERE PHONG.SOPHONG = PHONGTHUE.SOPHONG AND PHONGTHUE.MAKH = KHACHHANG.MAKH AND NGAYDEN < " + DateTime.Today.Year + "/" + DateTime.Today.Month + "/" + DateTime.Today.Day + "AND MADK = '" + madatphongtxt.Text + "' AND LOAIPHONG = '" + comboBox2.Text + "'";
                    showdata(sql);
                }
                catch
                {
